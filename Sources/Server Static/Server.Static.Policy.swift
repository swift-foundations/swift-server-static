...
extension Server.Static.Policy {
    ...
    private static func path(from requestPath: String) -> Result<String, Server.Static.Reason> {
        ...
        let leadingSlashRemoved =
            requestPath.first == "/" ? String(requestPath.dropFirst()) : requestPath
    ...
    private static func isUTF8(_ bytes: [UInt8]) -> Bool {
        ...
        while index < bytes.count {
            ...
            if first <= 0x7F {
                ...
                guard index + 1 < bytes.endIndex, 0x80...0xBF ~= bytes[index + 1] else {
                    return false
                }
                index += 2
            } else if first == 0xE0 {
                guard index + 2 < bytes.count, 0xA0...0xBF ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2]
                else { return false }
                index += 3
            } else if 0xE1...0xEC ~= first || 0xEE...0xEF ~= first {
                guard index + 2 < bytes.count, 0x80...0xBF ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2]
                else { return false }
                index += 3
            } else if first == 0xED {
                guard index + 2 < bytes.count, 0x80...0x9F ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2]
                else { return false }
                index += 3
            } else if first == 0xF0 {
                guard index + 3 < bytes.count, 0x90...0xBF ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2], 0x80...0xBF ~= bytes[index + 3]
                else { return false }
                index += 4
            } else if 0xF1...0xF3 ~= first {
                guard index + 3 < bytes.count, 0x80...0xBF ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2], 0x80...0xBF ~= bytes[index + 3]
                else { return false }
                index += 4
            } else if first == 0xF4 {
                guard index + 3 < bytes.count, 0x80...0x8F ~= bytes[index + 1],
                    0x80...0xBF ~= bytes[index + 2], 0x80...0xBF ~= bytes[index + 3]
                else { return false }
                index += 4
            } else {
