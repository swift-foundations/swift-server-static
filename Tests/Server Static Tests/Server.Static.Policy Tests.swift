...
extension Server.Static.Policy.Test.Unit {
    ...
    func `resolves root and trailing slash indexes`() {
        ...
        #expect(
            policy.resolve(.init(path: "/")) == .resource(.init(path: "index.html", kind: .index))
        )
        #expect(
            policy.resolve(.init(path: "/docs/"))
                == .resource(.init(path: "docs/index.html", kind: .index))
        )
    }
    ...
    func `resolves documentation and tutorial indexes`() {
        ...
        #expect(
            policy.resolve(.init(path: "/documentation/"))
                == .resource(.init(path: "documentation/index.html", kind: .index))
        )
        #expect(
            policy.resolve(.init(path: "/tutorial/"))
                == .resource(.init(path: "tutorial/index.html", kind: .index))
        )
    }
}
...
extension Server.Static.Policy.Test.`Edge Case` {
    ...
    func `decodes percent encoded paths`() {
        ...
        #expect(
            policy.resolve(.init(path: "/hello%20world.txt"))
                == .resource(.init(path: "hello world.txt"))
        )
        #expect(
            policy.resolve(.init(path: "/data/documentation.json"))
                == .resource(.init(path: "data/documentation.json", kind: .documentationJSON))
        )
    }
...
extension Server.Static.Policy.Test.Integration {
    ...
    func `resolves static assets by configured prefix`() {
        ...
        #expect(
            policy.resolve(.init(path: "/static/site.css"))
                == .resource(.init(path: "static/site.css", kind: .asset))
        )
        #expect(
            policy.resolve(.init(path: "/documentation.json"))
                == .resource(.init(path: "metadata/documentation.json", kind: .documentationJSON))
        )
        #expect(
            policy.resolve(.init(path: "/metadata/documentation.json"))
                == .resource(.init(path: "metadata/documentation.json", kind: .documentationJSON))
        )
    }
}
